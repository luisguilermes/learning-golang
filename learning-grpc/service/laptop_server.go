...
func (server *LaptopServer) UploadImage(
	...
	req, err := stream.Recv()
	if err != nil {
		log.Printf("cannot receive image info: %v", err)
	...
	for {
		...
		if imageSize > maxImageSize {
			return logError(status.Error(codes.InvalidArgument, "image is too large"))
		}
	...
	log.Printf("Image with ID %s and size %d saved to the store", imageID, imageSize)
	return nil
}
...
func (server *LaptopServer) RateLaptop(
	...
	for {
		...
		req, err := stream.Recv()
		if err == io.EOF {
			log.Print("no more rating")
			break
		}
		if err != nil {
			return logError(status.Errorf(codes.Unknown, "cannot receive rating: %v", err))
		}
		...
		err = stream.Send(res)
		if err != nil {
			return logError(status.Errorf(codes.Unknown, "cannot send rating response: %v", err))
		}
...
func contextError(ctx context.Context) error {
	...
	case context.Canceled:
		return logError(status.Error(codes.Canceled, "client cancelled the request"))

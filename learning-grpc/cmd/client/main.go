...
func uploadImage(laptopClient pb.LaptopServiceClient, laptopID string, imagePath string) {
	...
	if err != nil {
		log.Fatal("cannot receive response from upload image: ", err)
	}
...
func rateLaptop(laptopClient pb.LaptopServiceClient, laptopIDs []string, scores []float64) error {
	...
	go func() {
		for {
			res, err := stream.Recv()
			if err == io.EOF {
				log.Print("no more rating response")
				waitResponse <- nil
				return
			}
			if err != nil {
				waitResponse <- fmt.Errorf("cannot receive rate response: %v", err)
				return
			}
			log.Printf("received a rate-laptop response: %v", res)
		}
	}()
	...
	for i, laptopID := range laptopIDs {
		...
		err := stream.Send(req)
		if err != nil {
			return fmt.Errorf("cannot send rate request: %v - %v", err, stream.RecvMsg(nil))
		}
		log.Printf("sent a rate-laptop request: %v", req)
	}
